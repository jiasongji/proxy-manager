...
审计日期：2026-06-15
审计范围：A/B Shadowsocks 落地分流、多用户管理、流量/限额降级策略、安全升级与失败回退、root 执行体验、Proxy Lite 独立项目发布记录、脚本交互、CI、公开文档脱敏。
...
- [x] `shellcheck proxy-manager.sh`：本机通过 Docker `koalaman/shellcheck:stable` 执行通过；`proxy-lite` 由独立仓库 CI 覆盖。
- [x] CLI help smoke 通过：
...
- [x] 完整项目操作类 smoke 使用临时测试变量适配 root 门禁；`proxy-lite` 的 bash/shellcheck/help/menu/generated config/check/backup/rollback/upgrade smoke 在独立仓库 CI 中通过。
